...
Goal: given a Bug ID (e.g., BUG-001), analyze the bug file under `context/bugs/BUG-001.md` and produce a clear, actionable plan to debug and fix it. You must read the bug file (per-file format) and use only the information there plus optionally `context/to_do.md` and `context/tracker.json` to place the fix in project context. Never modify bug files; only produce a plan and optionally offer to append tasks via `/update_to_do` (with user confirm).
...
- File: `context/bugs/BUG-001.md` (must exist). If missing, ask user to confirm ID or create the bug via `/report_bug`.
- Optional: `context/to_do.md`, `context/tracker.json` for context enrichment.
...
7. **Integration with workflows**
   ...
   - Offer to append the development tasks to `context/to_do.md` by invoking `/update_to_do`. Do NOT append automatically — present the tasks and ask: "Append these to to_do? (yes/no)". If user agrees, call `/update_to_do` as a subtask (human-in-loop).
...
8. **Output the plan**
   - Present a compact plan in chat containing:
     ...
     - Optional next action: "Type `append` to add tasks to to_do.md or `done` to finish."
